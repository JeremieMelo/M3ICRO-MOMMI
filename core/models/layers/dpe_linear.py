...
class _DPELinearMultiPath(DPE_Layer_BASE):
    def __init__(
        self,
        in_features: int,
        out_features: int,
    ...
    def build_parameters(self, bias: bool) -> None:
        self.in_channels_pad = int(np.ceil(self.in_channels / self.mini_block).item() * self.mini_block)
        if self.unfolding:
        ...
        if self.sigma_trainable.startswith("row_col"):
            ...
            self.sigma = nn.Parameter(torch.ones(num_sigma, self.grid_dim_y, 1, self.mini_block, 2, device=self.device))
        elif self.sigma_trainable.startswith("col"):
            self.sigma = nn.Parameter(torch.ones(num_sigma, 1, self.grid_dim_x, self.mini_block, 2, device=self.device))
    ...
    def build_weight_unroll(self, weight: Tensor) -> Tensor:
        ...
        # weight_p, weight_n = weight.chunk(2, dim=0) # [p/2, q, k, k] complex
        # weight = weight_p + weight_n # [p/2, q, k, k] complex

        weight = weight.permute(0, 2, 1, 3).reshape(self.out_channels_pad // self.path_multiplier, -1)[
            : self.out_channels // 2, : self.in_channels
        ]
        weight = torch.view_as_real(weight)  # [outc/2, inc, 2]
        # weight = weight.permute(2,0,1).flatten(0,1)
        weight = weight.permute(0, 2, 1).flatten(0, 1)
        return weight
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        if self.unfolding:
            weight = self.build_weight_unroll(weight)
        else:
            weight = self.build_weight(weight)

        if torch.is_complex(weight):
            x = torch.complex(F.linear(x, weight.real), F.linear(x, weight.imag)).abs()
        else:
            x = F.linear(x, weight)
        ...
        # print("linear", x.data.mean(), x.data.std())
...
class DPELinear(DPE_Layer_BASE):
    _linear_types = _DPELinearMultiPath
    ...
    def __init__(
        ...
        self._fc_pos = _DPELinearMultiPath(
            in_features=self.in_channels_pos,
    ...
    def reset_parameters(self):
        for m in self.modules():
            if isinstance(m, self._linear_types):
                m.reset_parameters(fan_in=self.in_channels)
    ...
    def _forward_impl(self, x):
        ...
        if self._fc_neg is not None:
            y_neg = self._fc_neg(x)
            y = y - y_neg
